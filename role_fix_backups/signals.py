...
def student_id_card_handler(sender, instance, created, **kwargs):
    ...
    """
    if created:
        generate_id_card.delay(user_id=instance.id, primary_role='student')
    elif instance.user:
...
def teacher_id_card_handler(sender, instance, created, **kwargs):
    ...
    """
    if created:
        generate_id_card.delay(user_id=instance.id, primary_role='teacher')
    elif instance.user:
        regenerate_id_cards_on_profile_update.delay(user_id=instance.user.id)
...
def staff_admin_id_card_handler(sender, instance, created, **kwargs):
    ...
    """
    if instance.primary_role in ['admin', 'staff']:
        if created:
            generate_id_card.delay(user_id=instance.id, primary_role=instance.primary_role)
        else:
