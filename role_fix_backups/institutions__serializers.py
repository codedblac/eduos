...
from .models import Institution, SchoolAccount, SchoolRegistrationRequest, InstitutionType, SchoolType
...
class InstitutionMinimalSerializer(serializers.ModelSerializer):
    """Lightweight serializer for dropdowns and nested representations."""
    class Meta:
        model = Institution
        fields = ['id', 'code']
...
class InstitutionCreateUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Institution
        fields = [ 'code', 'registration_number',
            'country', 'county', 'sub_county', 'ward', 'village',
            'address', 'phone', 'email', 'website',
            'logo', 'primary_color', 'secondary_color', 'theme_mode',
            'school_type', 'institution_type', 'ownership', 'funding_source',
...
# ==========================
# 📝 School Registration Request Serializers
# ==========================

class SchoolRegistrationRequestSerializer(serializers.ModelSerializer):
    """
    Full serializer for admins to view, approve or reject school registration requests.
    """
    class Meta:
        model = SchoolRegistrationRequest
        fields = [
            'id', 'school_name', 'code', 'country', 'county', 'sub_county', 'ward', 'village',
            'email', 'phone', 'institution_type', 'school_type',
            'submitted_at', 'approved', 'approved_at', 'admin_created'
        ]
        read_only_fields = ['id', 'submitted_at', 'approved', 'approved_at', 'admin_created']


class SchoolRegistrationRequestCreateSerializer(serializers.ModelSerializer):
    """
    Serializer used by schools to submit registration requests.
    """
    class Meta:
        model = SchoolRegistrationRequest
        fields = [
            'school_name', 'code', 'country', 'county', 'sub_county', 'ward', 'village',
            'email', 'phone', 'institution_type', 'school_type'
        ]

    def validate_code(self, value):
        # Prevent duplicate codes in pending requests or approved institutions
