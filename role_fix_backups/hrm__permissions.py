...
class IsHRManager(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            getattr(request.user, 'primary_role', None) == 'hr_manager' or request.user.is_superuser
        )
...
class IsDepartmentHead(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user.primary_role== 'hr_manager' or user.is_superuser:
            return True
...
class IsSelfOrReadOnly(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            return obj.user == request.user or request.user.primary_role== 'hr_manager'
        return request.user.primary_role== 'hr_manager' or request.user.is_superuser
...
class IsAdminOrHR(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            request.user.is_superuser or request.user.primary_role== 'hr_manager'
        )
