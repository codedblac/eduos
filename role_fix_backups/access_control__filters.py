...
class UserRoleAssignmentFilter(django_filters.FilterSet):
    ...
    primary_role= django_filters.ModelChoiceFilter(queryset=Role.objects.all())
    institution = django_filters.ModelChoiceFilter(queryset=Institution.objects.all())
    is_active = django_filters.BooleanFilter()
    ...
    class Meta:
        ...
        fields = ['user', 'primary_role', 'institution', 'is_active', 'assigned_at', 'expires_at']
...
class RolePermissionFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = ['primary_role', 'permission', 'allow']
...
class RoleAuditLogFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = RoleAuditLog
        fields = ['actor', 'target_user', 'institution', 'primary_role', 'action', 'timestamp']
