...
class InstitutionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if getattr(user, 'primary_role', None) == 'SUPER_ADMIN':
...
class SchoolAccountListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if getattr(user, 'primary_role', None) != 'SUPER_ADMIN' and user.institution.id != uuid.UUID(institution_id):
...
class InstitutionAnalyticsOverviewView(views.APIView):
    ...
    def get(self, request):
        user = request.user
        queryset = Institution.objects.all()
        if getattr(user, 'primary_role', None) != 'SUPER_ADMIN' and getattr(user, 'institution', None):
