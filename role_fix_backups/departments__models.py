...
class DepartmentUser(models.Model):
    ...
    def __str__(self):
        return f"{self.user.get_full_name()} - {self.department.name} ({self.primary_role})"


class DepartmentRoleAssignmentHistory(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    department = models.ForeignKey(Department, on_delete=models.CASCADE)
    primary_role= models.CharField(max_length=20, choices=DepartmentUser.ROLE_CHOICES)
